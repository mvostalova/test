...
def get_valid_weight():
    ...
    while weight_invalid:
        try:
            weight = float(input("Zadejte svou hmotnost v kg: "))
            if weight > 0:
                weight_invalid = False
            else:
                print("Hmotnost musí být kladné číslo. Zkuste to znovu.")
        except ValueError:
...
def get_valid_gender():
    ...
    while sex.lower() not in ["male", "female"]:
        sex = input("Zadejte prosím své pohlaví (Male/Female): ").lower()

        if sex not in ["male", "female"]:
            print("Neplatný vstup. Zadejte prosím 'Male' nebo 'Female'.")
...
sex = get_valid_gender()

print(f"Vaše hmotnost je {weight} kg a vaše pohlaví je {sex.capitalize()}.")

"""
def generate_squats():
    # Tady by byla logika pro generování dřepů
    pass
"""
